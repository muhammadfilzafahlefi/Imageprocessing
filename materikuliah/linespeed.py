...
previous_centers = []  # Store previous centers of detected cars
speed_factor = 1.0 
...
while cap.isOpened():
    ...
    for (x, y, w, h) in cars:
        ...
        # Calculate speed if there was a previous position recorded
        if previous_centers:
            # Calculate distance in pixels
            distance = ((current_center[0] - previous_centers[0][0]) ** 2 + 
                         (current_center[1] - previous_centers[0][1]) ** 2) ** 0.5
        ...
        previous_centers.insert(0, current_center)
        if len(previous_centers) > 1:
            previous_centers.pop()  # Keep only the latest position
